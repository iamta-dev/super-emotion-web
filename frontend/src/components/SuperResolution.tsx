...
import { analyzeImg, superResolutionImg } from "../services/deepFace";
interface IDeepFace {
  instance_1: {
    age: number;
    dominant_emotion:
...
export const SuperResolution = ({ name }: { name: string }) => {
  const [processing, setProcessing] = useState<boolean>(false);
  const selectImgRef = useRef(null);
  const [selectImg, setSelectImg] = useState<string>("");
  const [resolutionPic, setResolutionPic] = useState<string>("");
  ...
  const resolutionImg = async (imgBase64: string) => {
    setProcessing(true);
    try {
      const res = await superResolutionImg(imgBase64);
      console.log(res);
      if (res !== undefined) {
        setResolutionPic(res.data.img);
      }
    } catch (error) {
      console.log(error);
    } finally {
  ...
  return (
    <>
      <div className="flex justify-center">
        ...
        >
          <div className="w-70vh min-h-80vh flex flex-col items-center space-y-4 bg-white rounded-xl shadow-lg p-4 mx-8 mt-8">
            <div className="flex text-center text-4xl font-bold">{name}</div>
            {selectImg !== "" && (
              <div className="grid place-items-center">
                <div className="">{/* ตัวอย่างรูป */}</div>
                <img
                  alt="slip"
            ...
            <div className="flex flex-row">
              <div className="flex justify-center">
                {selectImg !== "" && resolutionPic === "" && !processing && (
                  <div
            ...
            {instance.trx_id !== "" && <Summary instance={instance} />}
          </div>
        </form>
      </div>
    </>
  ...
  return (
    <>
      <div className="bg-white">
        <div className="px-4 py-2 flex justify-between font-bold text-4xl">
          ค่าจาก model
        </div>
        <div className="flex justify-between mb-8">
          <div className="grid h-48 w-full px-4 py-2">
